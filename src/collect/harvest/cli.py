"""collect-harvest — manueller Vault-Ingest von einer Quelle.

    collect-harvest wikipedia "HTTP protocol" --limit 300 --lang en
    collect-harvest wikipedia "TLS handshake" --limit 200 --dry-run
...
def _known_ids() -> set:
    """Vorhandene Doc-IDs — damit der Harvester sie gar nicht erst fetcht."""
...
def main() -> int:
    ...
    ap.add_argument("topic", help="Thema/Suchbegriff")
    ...
    ap.add_argument("--no-expand", action="store_true",
                    help="keine Link-Expansion (nur Such-Titel)")
    args = ap.parse_args()

    from collect.harvest.ingest import VaultIngest
    from collect.harvest.wikipedia import WikipediaSource

    print(f"Harvest: {args.source} '{args.topic}' (lang={args.lang}, "
          f"limit={args.limit}{', DRY-RUN' if args.dry_run else ''})")
    known = _known_ids()
    print(f"Vault kennt {len(known):,} Docs — bereits Vorhandenes wird übersprungen.")
